    ...
    fun preprocess e =
      case e of 
        LineSegment(x1, y1, x2, y2) =>
          if abs(x1 - x2) < epsilon andalso abs(y1 - y2) < epsilon then
            Point(x1, y1)
          else if x1 > x2 orelse (abs(x1 - x2) < epsilon andalso y1 > y2) then
            LineSegment(x2, y2, x1, y1)
          else
            LineSegment(x1, y1, x2, y2)
      | Intersect(e1, e2) =>
          Intersect(preprocess e1, preprocess e2)
      | Let(var, e1, e2) =>
          Let(var, preprocess e1, preprocess e2)
      | Shift(dx, dy, e) =>
          Shift(dx, dy, preprocess e)
      | _ => e
  in
    preprocess exp
  end
